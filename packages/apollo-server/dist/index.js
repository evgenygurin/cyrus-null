...
const resolvers = {
    Query: {
        ...
        myIssues: async (_parent, args, context) => {
            try {
        ...
        issueComments: async (_parent, args, context) => {
            try {
    ...
    Mutation: {
        createIssue: async (_parent, args, context) => {
            try {
        ...
        updateIssueState: async (_parent, args, context) => {
            try {
...
export async function startServer(port = 4000) {
    const { url } = await startStandaloneServer(server, {
        ...
        context: async ({ req }) => {
            ...
            const token = process.env.LINEAR_GRAPHQL_API_KEY || req.headers.authorization;
            if (!token) {
                throw new Error("Linear API token is required. Please set LINEAR_GRAPHQL_API_KEY environment variable.");
            }
    ...
    console.log(`🔑 Using Linear API token: ${process.env.LINEAR_GRAPHQL_API_KEY ? "✓ Configured" : "✗ Missing"}`);
    return { url, server };
}
